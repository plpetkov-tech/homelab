...
display_usage() {
    ...
    echo "  destroy              WARNING: DESTROYS ALL infrastructure, VMs, storage, and data for the current cluster"
...
case "$COMMAND" in
    ...
    upgrade-k8s)
        ...
        ;;
    update-metrics)
        ( "$REPO_PATH/scripts/update_control_plane_metrics.sh" "$@" )
        ;;
    health-check)
        ( "$REPO_PATH/scripts/cluster-health-check.sh" "$@" )
        ;;
    fix-gpu-operator)
